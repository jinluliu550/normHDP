...
normHDP_mcmc <- function(Y, 
  ...
  baynorm_ind <- lapply(1:D, function(d){
    bayNorm(Data = Y[[d]], BETA_vec = NULL, mode_version = TRUE, mean_version = FALSE, BB_SIZE = BB_SIZE)
  })
  
  #-- apply bayNorm to the combined dataset
  baynorm_tot <- bayNorm(Data = do.call(cbind, Y), BETA_vec = NULL, mode_version = TRUE, mean_version = FALSE, BB_SIZE = BB_SIZE)
  ...
  #-- estimate of beta
  baynorm.beta.estimate <- lapply(1:D, function(d) baynorm_ind[[d]]$PRIORS$BETA_vec)
  
  #-- estimate of standardized beta
  baynorm.beta.estimate.st <- lapply(1:D, function(d){
    ifelse(baynorm.beta.estimate[[d]]/mean(baynorm.beta.estimate[[d]])*beta.mean >= 1, 
           0.99,
           baynorm.beta.estimate[[d]]/mean(baynorm.beta.estimate[[d]])*beta.mean)})
  
  #-- ratio between the standardized beta and baynorm estimated beta
  baynorm.st.prop <- mean(unlist(lapply(1:D, function(d) mean(baynorm.beta.estimate.st[[d]]/baynorm.beta.estimate[[d]]))))
  
  #-- estimate of standardized mu
  baynorm.mu.estimate.st <- baynorm.mu.estimate/baynorm.st.prop
  
  #-- removed unwanted statistics
  rm(baynorm_ind, baynorm_tot, baynorm.mu.estimate, baynorm.beta.estimate, baynorm.st.prop)
  ...
  if(is.null(alpha_mu_2)){
    log.mu.finite.index <- which(is.finite(log(baynorm.mu.estimate.st)))
    alpha_mu_2 <- 2*log(mean(baynorm.mu.estimate.st[log.mu.finite.index]))
  }
  ...
  loop.result <- lapply(1:D, function(d){
    ...
    baynorm_mean_capeff <- mean(baynorm.beta.estimate.st[[d]])
  ...
  baynorm.phi.estimate[is.infinite(log(baynorm.mu.estimate.st))] <- NA
  baynorm.mu.estimate.st[is.infinite(log(baynorm.mu.estimate.st))] <- NA
  
  x <- log(baynorm.mu.estimate.st)
  ...
  baynorm.mu.estimate.st <- ifelse(baynorm.mu.estimate.st==0, 0.01, baynorm.mu.estimate.st)
  baynorm.phi.estimate <- ifelse(baynorm.phi.estimate==0, 0.01, baynorm.phi.estimate)
  
  mu_star_1_J_initial <- t(matrix(baynorm.mu.estimate.st, nrow=G, ncol=J))
  ...
  Beta_new <- baynorm.beta.estimate.st
